...
const { Case }  = require('../../case.js')
    ...
    main(processArgv,socket){
        console.log('case 3 - We confirm second user signUp successfully')

        let scope = this
        this.auxGetConfirmationToken( 'prueba-2@mailinator.com', processArgv ).then(response=>{
            
            let req3 = { query:{ ts:null }  }
    
            let res3 = new Emulate('case 6','We confirm second user signUp successfully','html','has being successfull signedUp', scope.case.resolves[scope.case.index], socket,5)
