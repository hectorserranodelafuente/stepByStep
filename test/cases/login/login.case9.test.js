...
class LoginCase9 {
    ...
    main( processArgv, socket ){
        ...
        let res5 = new Emulate(
            `case 17`,
            `We login with a user that has being logged before so,`+
            `HAVE SESSION INITIALIZED and NOT 2FA ACTIVATED, so it is redirected to init(in frontend)`,
            'jsonContains',
            {
                action:0,
                description:'Redirect to init'
